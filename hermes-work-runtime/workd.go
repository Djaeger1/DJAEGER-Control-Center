package main
import("archive/zip";"crypto/rand";"crypto/sha256";"crypto/tls";"crypto/x509";"encoding/hex";"encoding/json";"flag";"fmt";"io";"net";"net/http";"net/url";"os";"path/filepath";"sort";"strconv";"strings";"time")
...
func planCounts(a []PlanItem)map[string]int{m:=map[string]int{"IDEA_READY":0,"SCRIPT_PREP_READY":0,"SCRIPT_READY":0,"PRODUCTION_READY":0,"PRODUCTION":0,"PUBLISHED":0,"HOLD":0};for _,x:=range a{m[x.Stage]++};return m}
...
func validStage(x string)bool{switch x{case"IDEA_READY","SCRIPT_PREP_READY","SCRIPT_READY","PRODUCTION_READY","PRODUCTION","PUBLISHED","HOLD":return true};return false}
...
func (s *S)ensureProductionPacks()[]ProductionPack{
 ...
 for _,sp:=range scripts{
  ...
  if stage=="SCRIPT_READY"||stage=="PRODUCTION_READY"||stage=="PRODUCTION"||stage=="PUBLISHED"{
...
func (s *S)channel(w http.ResponseWriter,r *http.Request){
 ps:=s.performanceSummary();records,_:=ps["records"].(int)
 if records==0{js(w,map[string]any{"state":"NOT_CONNECTED","views":"NOT_AVAILABLE","retention":"NOT_AVAILABLE","ctr":"NOT_AVAILABLE","watch_time":"NOT_AVAILABLE","best_topic":"NOT_AVAILABLE","weak_topic":"NOT_AVAILABLE","feedback":"READY_WAITING_REAL_DATA"});return}
 ...
 js(w,map[string]any{"state":"FEEDBACK_CONNECTED","views":ps["views"],"retention":ps["avg_retention_pct"],"ctr":ps["avg_ctr_pct"],"watch_time":ps["watch_time_min"],"best_topic":best,"weak_topic":weak,"records":records,"engine":"FEEDBACK_V1"})
}
func (s *S)knowledge(w http.ResponseWriter,r *http.Request){
 ...
 js(w,map[string]any{"research_items":total,"unique_keys":len(seen),"categories":cats,"produced":ps["records"],"successful":ps["strong_signal"],"underperforming":ps["weak_signal"],"ideas_not_produced":notProduced,"feedback_state":ps["state"],"feedback_engine":"FEEDBACK_V1"})
}
...
<div class="box"><div class="k">Production Pack Ready</div><div id="productionready" class="v">-</div></div><div class="box"><div class="k">Production</div><div id="production" class="v">-</div></div>
...
async function plannerData(){
 try{
  ...
  $('scriptprepready').textContent=txt(cc.SCRIPT_PREP_READY??0);$('scriptsready').textContent=txt(cc.SCRIPT_READY??0);$('productionready').textContent=txt(cc.PRODUCTION_READY??0);$('production').textContent=txt(cc.PRODUCTION??0);$('published').textContent=txt(cc.PUBLISHED??0);$('nextscript').textContent=txt(p.next_for_script);
