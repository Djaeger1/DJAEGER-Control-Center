...
# r45 integration only added a compact summary and left the user with the
# GAME editor, which made the APP registry look missing.
...
# Keep the existing Session architecture, but make the canonical APP list
...
# ENGINE / SESSION used game-session ACTIVE as if it were the only workload
# activity.  Preserve that backend truth, but label APP/SYSTEM foregrounds
# accurately so WAITING GAME does not look like DJAEGER is inactive.
...
# Hard UI regression gates.  These are deliberately structural: the build
# must fail if APP registry disappears again or if APP/SYSTEM are presented
# as a game session.
M = main.read_text()
