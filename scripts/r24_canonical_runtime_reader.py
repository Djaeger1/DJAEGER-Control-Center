...
rs=r.read_text()
# R19 emits the marker as `echo __BRAIN__; cat ...`, so match the stable file read
# rather than assuming the generated Kotlin retains one exact echo spelling.
if '__RUNTIME_VERSION__' not in rs:
    anchors=["cat '$persistent/local_brain_state' 2>/dev/null", 'local_brain_state']
    pos=-1
    for marker in anchors:
        pos=rs.find(marker)
        if pos >= 0: break
    assert pos >= 0, 'local brain runtime anchor missing'
    line_start=rs.rfind('\n',0,pos)+1
    indent=re.match(r'\s*',rs[line_start:]).group(0)
    injection=indent+"echo __RUNTIME_VERSION__; cat '$persistent/runtime_version.env' 2>/dev/null\n"
    rs=rs[:line_start]+injection+rs[line_start:]
...
for x in ['runtime_version.env','local_brain_state','djaeger_thoughts.env','local_memory_status.env','session_safety.env','__RUNTIME_VERSION__']: assert x in R,x
