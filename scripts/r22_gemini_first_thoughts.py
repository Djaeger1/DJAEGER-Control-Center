...
# While SOURCE has not been published yet, keep the UI neutral with a simple dash.
pattern=r'val src=thoughtField\(s\.thoughts,"SOURCE"\)\.ifBlank\s*\{\s*"LOCAL_AI"\s*\}'
ms,n=re.subn(pattern, 'val src=thoughtField(s.thoughts,"SOURCE").ifBlank{"-"}', ms, count=1)
assert n==1, 'R22 source fallback anchor missing'
old_badge='val badge=if(src=="GEMINI") "✦ GEMINI • $status" else "◆ LOCAL AI • $status"'
new_badge='val badge=when(src){"GEMINI"->"✦ GEMINI • $status";"LOCAL_AI"->"◆ LOCAL AI • $status";else->"-"}'
...
assert 'else->"-"' in ms, 'R22 neutral source placeholder missing'
assert 'SOURCE UNAVAILABLE' not in ms, 'R22 must use dash instead of SOURCE UNAVAILABLE'
m.write_text(ms)
print('R22_THOUGHTS_PRIORITY=MODULE_PUBLISHED_SOURCE_ONLY')
print('R22_THOUGHTS_PLACEHOLDER=DASH_UNTIL_SOURCE_PUBLISHED')
