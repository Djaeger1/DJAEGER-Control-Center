...
rs=r.read_text()
# Rebuild module-side key pool using the repository's proven su/suStdin helpers.
# Raw keys travel only over stdin and never appear in command arguments or cc_snapshot.
...
# Add explicit sync beside the vault controls. It is user-triggered only; never part of 1s refresh.
# Do not depend on later local UI state variables (bridgeMessage/bridgeBusy/scope).
...
if pos>=0:
    ...
    p=ms.find(marker); assert p>=0
    q=ms.find('\n',p)
    ms=ms[:q+1]+'                Button(onClick={ val keys=vault4.configuredKeys(); kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Main).launch { repo.syncGeminiKeyPool(keys,vault4.active()); vaultEpoch++ } }, enabled=vault4.configuredKeys().isNotEmpty()){ Text("SYNC 4-KEY POOL") }\n'+ms[q+1:]
m.write_text(ms)
