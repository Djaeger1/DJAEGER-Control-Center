...
# Add an explicit live clock row to Overview using the already-proven R18 persistent
# root fast-telemetry stream. This restores the CPU/GPU MHz values independently
# of game-session CSV freshness.
...
overview_anchor='};ThermalRow(s);BoxCard("PERFORMANCE"'
assert overview_anchor in ms,'Overview thermal anchor missing'
ms=ms.replace(overview_anchor,'};LiveClockRow();ThermalRow(s);BoxCard("PERFORMANCE"',1)
