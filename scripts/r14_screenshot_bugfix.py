...
ms=m.read_text()
# Header must remain readable in a narrow floating landscape window.
ms=ms.replace('CONTROL CENTER • v0.12.1-r13 • COMPLETE USER DESIGN • REALTIME 1s','CONTROL CENTER • v0.12.1-r14 • REALTIME 1s')
# The screenshot exposed a black-on-near-black DJAEGER wordmark. Force theme-aware high contrast.
...
# Cover alternative generated title formatting without touching other text.
if 'Text("DJAEGER",fontWeight=FontWeight.Black,fontSize=32.sp,color=' not in ms:
    ms=ms.replace('Text("DJAEGER",fontWeight=FontWeight.Black,fontSize=32.sp)', 'Text("DJAEGER",fontWeight=FontWeight.Black,fontSize=32.sp,color=MaterialTheme.colorScheme.onBackground)',1)
# Runtime module identity is factual. If a retired r78 is still installed, mark it explicitly instead of making it look like CC branding.
...
# Avoid accidental legacy branding in the app-owned header; runtime module value is intentionally preserved.
