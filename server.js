...
const release='DJAEGER_AI_CORE_CREDENTIAL_RESTORE1';
...
function num(v,min,max){if(v==null||v===''||String(v).toUpperCase()==='NA')return null;const x=Number(v);if(!Number.isFinite(x)||x<min||x>max)throw Object.assign(new Error('invalid_number'),{code:422});return x;} function telemetryNum(v,min,max){try{return num(v,min,max);}catch{return null;}}
function normalize(x){if(!x||typeof x!=='object'||Array.isArray(x))throw Object.assign(new Error('invalid_payload'),{code:400});return {schema:str(x.schema,64)||'DJAEGER_AI_TELEMETRY_V1',device_id:str(x.device_id,96),at:telemetryNum(x.at,0,4102444800)||Math.floor(Date.now()/1000),module_version:str(x.module_version,96),module_version_code:str(x.module_version_code,32),workload_class:str(x.workload_class,32),window_mode:str(x.window_mode,32),package:str(x.package??x.subject_package,160),cpu_little_khz:telemetryNum(x.cpu_little_khz,0,5000000),cpu_big_khz:telemetryNum(x.cpu_big_khz,0,5000000),gpu_hz:telemetryNum(x.gpu_hz,0,3000000000),skin_temp_c:telemetryNum(x.skin_temp_c,-30,150),battery_temp_c:telemetryNum(x.battery_temp_c,-30,100),cpu_temp_c:telemetryNum(x.cpu_temp_c,-30,150),gpu_temp_c:telemetryNum(x.gpu_temp_c,-30,150),power_mw:telemetryNum(x.power_mw,0,50000),fps:telemetryNum(x.fps,0,1000),jank_pct:telemetryNum(x.jank_pct,0,100),credential_recovery_state:str(x.credential_recovery_state,32),gemini_key_count:telemetryNum(x.gemini_key_count,0,4),hermes_access_key_present:str(x.hermes_access_key_present,8),hermes_endpoint_present:str(x.hermes_endpoint_present,8),hermes_cloud_id_present:str(x.hermes_cloud_id_present,8),neuron_local_estimate_present:str(x.neuron_local_estimate_present,8),neuron_used_est:telemetryNum(x.neuron_used_est,0,1000000000),neuron_limit:telemetryNum(x.neuron_limit,0,1000000000),neuron_epoch_day:telemetryNum(x.neuron_epoch_day,0,100000000),source:str(x.source,64)||'LOCAL_OBSERVER',server_received_at:now()};}
function push(a,v){a.unshift(v);if(a.length>MAX)a.length=MAX;}
function updateRootForVc(vc){return ['202','203','209','210'].includes(vc)?ADAPTIVE_UPDATE_ROOT:UPDATE_ROOT;}
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');
 if(req.method==='GET'&&u.pathname==='/health'){const age=lastDeviceReceivedMs?Math.max(0,Math.floor((Date.now()-lastDeviceReceivedMs)/1000)):null;const pollAge=lastUpdatePollMs?Math.max(0,Math.floor((Date.now()-lastUpdatePollMs)/1000)):null;const authAge=lastAuthRejectMs?Math.max(0,Math.floor((Date.now()-lastAuthRejectMs)/1000)):null;return send(res,200,{ok:true,service:'DJAEGER_AI_CORE',release,hardware_authority:'NONE',foreign_runtime_dependencies:0,uptime_sec:Math.floor((Date.now()-startedAtMs)/1000),telemetry_seen:lastDeviceReceivedMs>0,telemetry_accepted:telemetryAccepted,last_telemetry_age_sec:age,last_device_id_hash:lastDevice?deviceHash(lastDevice.device_id):null,last_source:lastDevice?.source||null,last_module_version_code:lastDevice?.module_version_code||null,credential_recovery_state:lastDevice?.credential_recovery_state||null,gemini_key_count:lastDevice?.gemini_key_count??null,hermes_access_key_present:lastDevice?.hermes_access_key_present||null,hermes_endpoint_present:lastDevice?.hermes_endpoint_present||null,hermes_cloud_id_present:lastDevice?.hermes_cloud_id_present||null,neuron_local_estimate_present:lastDevice?.neuron_local_estimate_present||null,neuron_used_est:lastDevice?.neuron_used_est??null,neuron_limit:lastDevice?.neuron_limit??null,update_poll_seen:lastUpdatePollMs>0,update_poll_count:updatePolls,last_update_poll_age_sec:pollAge,update_ack_count:updateAcks,auth_reject_count:authRejects,last_auth_reject_age_sec:authAge});}
 if(req.method==='GET'&&u.pathname==='/ready')return send(res,200,{ok:true,ready:true,release,adaptive_update_channel:true,adaptive_target_vc:['202','203','209','210']});
 ...
 if(req.method==='POST'&&u.pathname==='/v1/device/telemetry'){const t=normalize(await readJson(req));lastDevice=t;lastDeviceReceivedMs=Date.now();telemetryAccepted++;push(samples,t);console.log(JSON.stringify({event:'DJAEGER_AI_TELEMETRY_ACCEPTED',accepted:telemetryAccepted,device_id_hash:deviceHash(t.device_id),source:t.source,module_version_code:t.module_version_code,package:t.package,credential_recovery_state:t.credential_recovery_state,gemini_key_count:t.gemini_key_count,hermes_access_key_present:t.hermes_access_key_present,hermes_endpoint_present:t.hermes_endpoint_present,hermes_cloud_id_present:t.hermes_cloud_id_present,neuron_local_estimate_present:t.neuron_local_estimate_present,neuron_used_est:t.neuron_used_est,neuron_limit:t.neuron_limit,at:now()}));return send(res,200,{ok:true,accepted:true,hardware_action:false});}
...
if(require.main===module){
  ...
  setInterval(()=>{
    ...
    console.log(JSON.stringify({event:'DJAEGER_AI_TELEMETRY_HEARTBEAT',release,telemetry_seen:lastDeviceReceivedMs>0,telemetry_accepted:telemetryAccepted,last_telemetry_age_sec:age,last_device_id_hash:lastDevice?deviceHash(lastDevice.device_id):null,last_source:lastDevice?.source||null,last_module_version_code:lastDevice?.module_version_code||null,credential_recovery_state:lastDevice?.credential_recovery_state||null,gemini_key_count:lastDevice?.gemini_key_count??null,hermes_access_key_present:lastDevice?.hermes_access_key_present||null,hermes_endpoint_present:lastDevice?.hermes_endpoint_present||null,hermes_cloud_id_present:lastDevice?.hermes_cloud_id_present||null,neuron_local_estimate_present:lastDevice?.neuron_local_estimate_present||null,uptime_sec:Math.floor((Date.now()-startedAtMs)/1000),hardware_authority:'NONE',at:now()}));
  },60000).unref();
}
