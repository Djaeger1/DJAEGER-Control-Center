...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');
 ...
 if(req.method==='GET'&&u.pathname==='/v1/device/update/status')return send(res,200,{ok:true,channel:'stable',manifest_present:fs.existsSync(path.join(UPDATE_ROOT,'manifest.txt')),last_ack:lastUpdateAck,at:now()});
 ...
 if(req.method==='GET'&&u.pathname==='/v1/config')return send(res,200,{ok:true,release,cloud_hardware_authority:'NONE',device_executor_authority:'LOCAL_VALIDATED_ONLY',remote_hardware_commands:false,remote_software_updates:true,update_channel:'stable'});
