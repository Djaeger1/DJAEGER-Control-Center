...
const release='DJAEGER_AI_CLEAN_CORE_V1';
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');
 if(req.method==='GET'&&u.pathname==='/health')return send(res,200,{ok:true,service:'DJAEGER_AI_CORE',release,hardware_authority:'NONE',foreign_runtime_dependencies:0});
 ...
 if(req.method==='POST'&&u.pathname==='/v1/device/telemetry'){const t=normalize(await readJson(req));lastDevice=t;push(samples,t);return send(res,200,{ok:true,accepted:true,hardware_action:false});}
