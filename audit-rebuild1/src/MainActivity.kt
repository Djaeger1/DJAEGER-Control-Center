...
@Composable fun AgentRebuild1Card(s:RuntimeState){
...
@Composable fun HermesCard(s:RuntimeState){
    ...
    val displayState=when(hState){
        "VALID"->if(currentBrain=="HERMES_H2"||currentBrain=="HERMES_LOCAL") "SELECTED BY AGENT" else "READY"
        "STANDBY_GEMINI_ONLINE"->"STANDBY • GEMINI VALID"
        "LOW_CONFIDENCE"->"LEARNING • LOW CONFIDENCE"
        "UNAVAILABLE"->"UNAVAILABLE"
        else->hState
    }
    val authority="REASONING BRAIN • AI AGENT OWNS FINAL ARBITRATION/EXECUTION ORCHESTRATION"
