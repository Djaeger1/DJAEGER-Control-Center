...
@Composable fun StatusCard(s:RuntimeState){
    ...
    val visibleGame=envField(s.workloadContext,"VISIBLE_GAME").ifBlank{s.game.takeUnless{it=="NA"||it=="NONE"}?:"NONE"}
    ...
    val window=when{
        ...
        gameVisible->"MULTIWINDOW"
        else->"INACTIVE"
    }
    val profile=when{
        stale->"—"
        gameEngineActive->s.telemetry.profile
        workloadClass=="APP"||workloadClass=="SYSTEM"->workloadProfile
        else->"—"
    }
    val gameExecution=if(gameEngineActive) "ACTIVE" else "BLOCKED / OBSERVE ONLY"
