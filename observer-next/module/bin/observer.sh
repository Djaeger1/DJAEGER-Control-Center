...
while true; do
  ...
  # Do not pass decimal output through temp_c() a second time.
  SKIN=$(thermal_by_type 'sdm-skin-therm-usr|skin|shell|surface|quiet')
  ...
  TMP="$SNAP.tmp.$$"
  {
    ...
    echo "MODULE_VERSION=1.1.3-runtimefix"
    echo "SAMPLE_SEQ=$SEQ"
  ...
  TMP="$SNAP.tmp.$$"
  {
    echo "SCHEMA=DJAEGER_OBSERVER_V4"
    echo "ENGINE=OBSERVER_FIRST"
    echo "MODULE_VERSION=1.1.2-credrestore"
    echo "SAMPLE_SEQ=$SEQ"
    echo "PACKAGE_SAMPLES=$PKG_SAMPLES"
    echo "EPOCH=$EPOCH"
    echo "ACTIVE_PACKAGE=$PKG"
