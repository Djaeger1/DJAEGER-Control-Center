...
if [ -r "$BIN_DIR/singleton.sh" ]; then
  ...
  djaeger_singleton_claim shadow_contextual_v4
fi
...
OUT="$ROOT/runtime/shadow_contextual_v4.env"
APPROVAL="$ROOT/policy/approved_contextual_v4.env"
REJECT_LEDGER="$ROOT/history/shadow_reject_strategy.env"
CONTEXTUAL_LOCK="$ROOT/runtime/locks/shadow_contextual_v4.lock/pid"
CONTEXTUAL_SELF="$(cat "$CONTEXTUAL_LOCK" 2>/dev/null)"
case "$CONTEXTUAL_SELF" in ''|*[!0-9]*) exit 0;; esac
contextual_still_owns_lock(){ [ "$(cat "$CONTEXTUAL_LOCK" 2>/dev/null)" = "$CONTEXTUAL_SELF" ]; }
...
publish(){
  ...
  t="$OUT.tmp.$$"
  {
    echo "SHADOW_SCHEMA=DJAEGER_CONTEXTUAL_SHADOW_V4"
    echo "WORKER_VERSION=CONTEXTUAL_SHADOW_V4"
  ...
  if [ "$_state" = PASS ] && [ -r "$POLICY" ]; then
    _a="$APPROVAL.tmp.$$"
    {
      echo "SCHEMA=DJAEGER_EXEC_APPROVAL_CONTEXTUAL_V4"
      echo "AT=$_now"
      echo "EXPIRES_AT=$((_now+120))"
      echo "EXECUTOR_ALLOWED=YES"
      echo "SHADOW_SOURCE=CONTEXTUAL_V4"
      echo "PACKAGE=$(kv PACKAGE "$POLICY")"
      echo "INTENT=$(kv INTENT "$POLICY")"
      echo "ACTUATORS=$(kv ACTUATORS "$POLICY")"
...
while true; do
  contextual_still_owns_lock || exit 0
