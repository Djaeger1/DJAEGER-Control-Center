...
teacher_needed(){
  ...
  case "$1" in
    local_frame_recovery_boost_not_supported_by_history|all_local_comfort_strategies_quarantined|shadow_rejected_strategy_quarantine|healthy_local_observe_no_cloud_needed|human_comfort_hard_thermal_guard_wait_native_cooling|local_synth_hard_thermal_guard|cloud_takeover_deferred_hard_thermal_guard|local_memory_reuses_proven_strategy|local_synth_LOCAL_FRAME_CRITICAL_RECOVERY|local_synth_LOCAL_FRAME_CRITICAL_RECOVERY_STAGE1_BIG_GPU|local_synth_LOCAL_FRAME_DEGRADED_RECOVERY|local_synth_LOCAL_HUMAN_COMFORT_THERMAL_TRIM_GPU|local_synth_LOCAL_HUMAN_COMFORT_THERMAL_TRIM_BIG|local_synth_LOCAL_HUMAN_COMFORT_THERMAL_TRIM_LITTLE|local_synth_LOCAL_POWER_TRIM_GPU|local_synth_LOCAL_POWER_TRIM_BIG|local_synth_LOCAL_POWER_TRIM_LITTLE|local_synth_LOCAL_MATURE_MODEL_EFFICIENCY_PROBE|cloud_requests_observe|one_hermes_cloud_takeover_ready|gemini_offline_local_model_no_action_needed|local_synth_no_action_needed|no_safe_takeover_strategy|neuron_guard_min_interval|restart_guard_no_cloud|waiting_for_brain_state|non_game_workload|gemini_not_ready_not_failed|gemini_online_observe_or_rate_guard|cloud_health_reachable_auth_unverified|health_endpoint_auth_policy_*)
      return 1 ;;
    *) return 0 ;;
  esac
}
