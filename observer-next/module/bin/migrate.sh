#!/system/bin/sh
# DJAEGER AI credential/identity migration.
# Imports only explicitly allowlisted DJAEGER AI fields. Secret values are
# never written to migration.env or UI snapshots.
...
mkdir -p "$ROOT/recovery" "$RUNTIME" "$CONFIG"
...
STATE=NO_LEGACY
LEGACY_PRESENT=NO
TMP_KEYS="$RUNTIME/.migrate_gemini.$$"
TMP_SOURCES="$RUNTIME/.migration_sources.$$"
: > "$TMP_KEYS"
: > "$TMP_SOURCES"
chmod 600 "$TMP_KEYS" "$TMP_SOURCES" 2>/dev/null

# Preserve already-migrated keys first.
sed -n 's/^KEY_[1-4]=//p' "$GEMINI" 2>/dev/null | while IFS= read -r _k; do
  append_key "$_k"
done

# Real legacy format: raw one-key-per-line vault.
...
# Active legacy Gemini key is also a valid recovery source.
...
# Legacy Railway aliases are only accepted from exact DJAEGER Railway files.
...
# Explicit known Hermes credential backups.
for _f in \
  "$LEGACY/HERMES_CLOUD_CREDENTIALS.txt" \
  ...
  /sdcard/Download/HERMES_CLOUD_CREDENTIALS.txt
do
  [ -r "$_f" ] || continue
  LEGACY_PRESENT=YES
  STATE=LEGACY_FOUND
  ...
  _base="$(extract_exact ENDPOINT "$_f")"
  case "$_base" in https://*) [ -n "$H_BASE" ] || H_BASE"";; esac
done

# Exact legacy Hermes file can use HERMES_SHARED_TOKEN + base ENDPOINT.
if [ -r "$LEGACY/hermes_cloud.conf" ]; then
  ...
  _base="$(extract_exact ENDPOINT "$LEGACY/hermes_cloud.conf")"
  case "$_base" in https://*) [ -n "$H_BASE" ] || H_BASE="$_base";; esac
  _hep="$(extract_exact HERMES_ENDPOINT "$LEGACY/hermes_cloud.conf")"
  case "$_hep" in
    https://*) [ -n "$H_ENDPOINT" ] || H_ENDPOINT="$_hep" ;;
    /*) [ -n "$H_PATH" ] || H_PATH="$_hep" ;;
  esac
...
# Normalize base + relative path only when no complete Hermes URL is present.
case "$H_ENDPOINT" in
  https://*) : ;;
  *)
    case "$H_BASE" in
      https://*)
        H_BASE="$(printf '%s' "$H_BASE" | sed 's:/*d::(')"
        case "$H_PATH" in
...
# Deduplicate in stable order and keep exactly four Gemini keys.
...
# Restore per-key cooldowns from old slot|until|reason ledger.
...
# Preserve old local Neuron accounting as an estimate, never as provider quota.
NEURON_PRESENT=NO
    ...
    *)
      {
        ...
        case "$N_FAST" in ''|*[!0-9]*) :;; *) echo "FAST_CALLS=$N_FAST";; esac
        case "$N_SMART" in ''|*[!0-9]*) :;; *) echo "SMART_CALLS=$N_SMART";; esac
        case "$N_DEEP" in ''|*[!0-9]*) :;; *) echo "DEEP_CALLS=$N_DEEP";; esac
...
[ -s "$IDENTITY" ] && ICOUNT=1
RCOUNT=0
[ -s "$RAILWAY" ] && RCOUNT="$(awk -F= 'NF>=2{n++}END{print n+0}' "$RAILWAY")"

if [ "$GCOUNT" -gt 0 ] || [ "$HCOUNT" -gt 0 ] || [ "$ICOUNT" -gt 0 ] || [ "$RCOUNT" -gt 0 ]; then
  STATE=CLEAN_IMPORT_CREATED
else
