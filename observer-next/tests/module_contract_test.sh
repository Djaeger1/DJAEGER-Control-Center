...
SYNC_OUT=$(run_ctl sync-request 109 DJAEGER_AI_ADAPTIVE_V3 contract-test-1)
...
grep -Fqx 'MODULE_VERSION_CODE=208' "$SNAPSHOT"
grep -Fqx 'CONTROL_CENTER_VERSION_CODE=109' "$SNAPSHOT"
...
grep -Fq 'pkill -f "/djaeger_ai_observer/bin/$n.sh"' "$MODULE/service.sh"
grep -Fq 'STARTUP_SCHEMA=DJAEGER_STARTUP_V1' "$MODULE/service.sh"
