...
var BASE = process.env.MCP_SERVER_URL || "https://mcp.desktopcommander.app";
var SESSION_PATH = process.env.DJAEGER_RDC_SESSION || "/data/adb/modules/djaeger_remote_boot/state/session.json";
var STATUS_PATH = process.env.DJAEGER_STATUS_PATH || "/data/adb/modules/djaeger_remote_boot/state/status.env";
var DEVICE_NAME = process.env.DJAEGER_DEVICE_NAME || "DJAEGER-Android";
var VERSION = "1.0.0-djaeger-android";
var CLIENT_ID = "mcp-device";
var HEARTBEAT_MS = 5 * 60 * 1e3;
var HEALTH_MS = 15 * 1e3;
var CHANNEL_GRACE_MS = 45 * 1e3;
var log = (...a) => console.log((/* @__PURE__ */ new Date()).toISOString(), ...a);
var sleep = (ms) => new Promise((r) => setTimeout(r, ms));
var shuttingDown = false;
function setStatus(state, detail = "") {
  try {
    fs.mkdirSync(new URL(".", "file://" + STATUS_PATH).pathname, { recursive: true, mode: 448 });
  } catch {
  }
  try {
    fs.writeFileSync(
...
function saveSaved(saved) {
  ...
  fs.writeFileSync(SESSION_PATH, JSON.stringify(saved, null, 2), { mode: 384 });
}
async function deviceAuthorize() {
  ...
  const interval = Math.max(2, auth.interval || 5) * 1e3;
  const deadline = Date.now() + auth.expires_in * 1e3;
...
async function runCommand(args) {
  ...
  const timeout = Math.max(1e3, Math.min(Number(args?.timeout_ms ?? 1e4), 3e5));
...
async function executeTool(name, args) {
  if (name === "ping") return resultText("pong " + (/* @__PURE__ */ new Date()).toISOString());
  ...
  if (name === "shutdown") {
    ...
    return resultText("Shutdown initialized at " + (/* @__PURE__ */ new Date()).toISOString());
  }
...
async function main() {
  ...
  const { data: device, error: devErr } = await client.from("mcp_devices").select("id,user_id").eq("id", deviceId).eq("user_id", user.id).maybeSingle();
  ...
  async function finish(id, status, result = null, errorMessage = null) {
    const update = { status, completed_at: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  async function handleDoorbell(payload) {
    ...
    const { data, error } = await client.from("mcp_remote_calls").update({ status: "executing" }).eq("id", callId).eq("device_id", deviceId).eq("status", "pending").select("*");
    if (error) {
