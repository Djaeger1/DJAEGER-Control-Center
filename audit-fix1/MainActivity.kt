...
private fun brainLabel(raw:String):String = when(raw){
    "GEMINI"->"GEMINI"
    "HERMES_LOCAL"->"HERMES"
    "LOCAL_LEARNED"->"HERMES H2 • LEARNED"
    "LOCAL_BASELINE","LOCAL_FRAME","VALIDATOR_FALLBACK","RESCUE_GUARD"->"HERMES H2"
    else->if(raw.startsWith("LOCAL")) "HERMES H2" else raw.ifBlank{"—"}
}
...
@Composable fun ThoughtsCard(s:RuntimeState){
    ...
    val thoughtTitle=when{
        ...
        thoughtSrc=="LOCAL_AI"->"PEMIKIRAN HERMES H2"
...
@Composable fun AI(s:RuntimeState){
    ...
    val supported=s.moduleVersion.contains("DUALBRAIN-AGENT-A2-SYNC1"); val retained=humanDecision(s); val brainText=if(s.brain.isBlank()) "No Hermes H2 state published.\nLAST SESSION:\n"+retained else s.brain
    Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)){
