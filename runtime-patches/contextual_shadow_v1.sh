...
S="$M/bin/shadow.sh"
W="$R/runtime/contextual_shadow_v1"
N="$W/shadow.new"
SNIP="$W/contextual.snip"
mkdir -p "$W" || exit 1

cat > "$SNIP" <<'EOF'
  # CONTEXTUAL_SHADOW_V1
...
END_COUNT=$(grep -c '^  set -- $METRICS$' "$S" 2>/dev/null || true)

echo "METRICS_START_MARKERS=$START_COUNT"
echo "METRICS_END_MARKERS=$END_COUNT"
...
awk '
  BEGIN{drop=0}
  {
    if($0 ~ /^  WINDOWS=\\$\\{1:-0\\}; FPS_AVG=/){drop=1; next}
    if(drop && $0 ~ /^  INTENT=\\$\\(kv INTENT /){drop=0; print; next}
    if(drop) next
    print
  }
' "$N" > "$W/shadow.cleaned" || exit 44
mv -f "$W/shadow.cleaned" "$N"

if ! grep -Fq 'SHADOW_CONTEXT_TIER=' "$N"; then
  awk '
    {
      print
      if($0=="    echo \\"SHADOW_POWER_WINDOWS=${POWER_N:-0}\\"" && !done){
    ...
    END{if(!done) exit 45}
  ' "$N" > "$W/shadow.publish" || exit 45
  mv -f "$W/shadow.publish" "$N"
fi

sh -n "$N" || exit 51
grep -Fq "CONTEXTUAL_SHADOW_V1" "$N" || exit 52
...
TS=$(date +%s)
cp "$S" "$R/runtime/shadow.pre_contextual_v1.$TS" || exit 61
cp "$N" "$S" || exit 62
chmod 755 "$S"
...
echo "PATCH=CONTEXTUAL_SHADOW_V1_ACTIVE"
