...
worthy(){
  case "$1" in waiting_for_brain_state|non_game_workload|gemini_not_ready_not_failed|gemini_online_observe_or_rate_guard|cloud_health_reachable_auth_unverified|health_endpoint_auth_policy_*|restart_guard_no_cloud|neuron_guard_min_interval) return 1;; *) return 0;; esac
}
cloud_align(){
  ...
  if [ "$http" = 200 ] && [ -r "$resp" ]; then
    ...
    taught=$(printf '%s\n' "$payload" | sed -n 's/.*THOUGHT=//p' | head -n1); taught=$(clean "$taught")
    if [ -n "$taught" ]; then
